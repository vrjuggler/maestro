...
__revision__ = "$Id: process.py 551 2005-09-26 19:01:45Z trentm $"
...
def _waitpidRetryOnEINTR(pid, options):
    ...
    From 'man 2 waitpid':
        [EINTR]     The call was interrupted by a caught signal, or
                    the signal did not have the SA_RESTART flag set.
    """
    while 1:
        try:
...
def _whichFirstArg(cmd, env=None):
    ...
    Raise a ProcessError or which.WhichError if no such executable could
    be found.
    """
...
class _FileWrapper:
    ...
    def write(self, text):
        if self._descriptor is not None:
            _writeRetryOnEINTR(self._descriptor, text)
        elif self._handle is not None:
            try:
    ...
    def wait(self, timeout=None): 
        ...
        if sys.platform.startswith("win"):
            ...
            if self.__retvalCache is not None:
                ...
                if timeout is None:
                    timeout = 0
                pid, sts = _waitpidRetryOnEINTR(self._pid, timeout)
                if pid == self._pid:
